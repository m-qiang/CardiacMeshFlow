...
def train_loop(args):
    ...
    for epoch in tqdm(range(n_epoch+1)):
        ...
        for idx, data in enumerate(trainloader):
            subj_idx, _, ffd1_gt, ffd2_gt, ffd3_gt = data
            ...
            for j in np.random.permutation(np.arange(n_frame)):
                ...
                dz1_t, dz2_t, dz3_t = cardiac_mesh_flow(z1_t, z2_t, z3_t, t)
        ...
        if epoch % 5 == 0:
            ...
            torch.save(
                noise_sampler.state_dict(),
                './ckpts/model_noise_sampler_'+tag+'_'+str(epoch)+'epochs.pt')
            torch.save(
                fusion.state_dict(),
                './ckpts/model_fusion_'+tag+'_'+str(epoch)+'epochs.pt')
    
            # ------ gt volume distribution ------
            volume_list_gt = []
            for idx, data in enumerate(validloader):
                _, volume_gt = data
                # normalise the four-chamber volume by EDV
                volume_gt_ed = volume_gt[0].sum(0)[0]
                volume_gt = volume_gt / volume_gt_ed * 5
                volume_list_gt.append(volume_gt.reshape(-1))
        
            volume_list_gt = torch.vstack(volume_list_gt).T
            mu_volume_gt = volume_list_gt.mean(-1).cpu().numpy()
            cov_volume_gt = volume_list_gt.cov().cpu().numpy()
    
            # ------ generate random 3D+t meshes ------
            volume_list_pred = []
            n_step = 1
            h = 1 / n_step
            for i in range(n_valid):  # 100 samples
                # fixed noise and phenotypes for each sample
                eps = noise_sampler.sample()
                ...
                volume_pred = np.vstack([volume_lv, volume_my, volume_rv, volume_la, volume_ra])
    
                # normalise the four-chamber volume by EDV
                volume_pred_ed = volume_pred.sum(0)[0]
                volume_pred = volume_pred / volume_pred_ed * 5
                volume_list_pred.append(volume_pred.reshape(-1))
    
            volume_list_pred = torch.Tensor(np.vstack(volume_list_pred).T).to(device)
            mu_volume_pred = volume_list_pred.mean(-1).cpu().numpy()
            cov_volume_pred = volume_list_pred.cov().cpu().numpy()
    
            # compute volumetric FID
            valid_fid = vFID(mu_volume_pred, mu_volume_gt, cov_volume_pred, cov_volume_gt)
    
            logging.info("------------------------------")
            logging.info("epoch:{}, valid vFID:{}".format(epoch, valid_fid))
            logging.info("------------------------------")
